...
# XXD Panel 140｜Dry-media Visual Island
...
Below, extract the most recognisable subject, contours, structure, pose and narrative relationships, make fresh artistic decisions and distil a small centered vignette / visual island. Do not copy the complete photograph or redraw every object. Boldly delete the background and secondary information, retaining only a few elements embodying the scene’s spirit, directional movement and visual memory. Even with clutter or a tiny subject, crop, regroup, resize and reposition freely to create an independently convincing artwork.
...
The refined, soothing effect comes from immense white paper, a tiny visual island, lively dry-media colour shapes, coarse grain, irregular edges and miniature handwriting. The point is not to shrink a painted photograph, but to distil its whole world into the memory most worth preserving at the centre of white paper.
...
- `design-only`: the full canvas contains only Panel 140's designed translation; the photograph remains a non-visible reference.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-140.git
npx skills add https://github.com/nevertoday/xxd-panel-140 --skill xxd-panel-140
```

Restart the agent session after installation, then invoke `$xxd-panel-140`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-140 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-140 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-140 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-140 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-140-prompt.en.md) or [Chinese](references/xxd-panel-140-prompt.zh-CN.md) runtime adapter.
