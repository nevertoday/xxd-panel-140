...
以下样张均来自不同的原始参考图，由 Panel 140 独立单轮生成，并已清理 AI 元数据。横版严格为左侧原图、右侧设计，各占 50%；竖版严格为上方原图、下方设计，各占 50%。
...
下半部分只提取照片中最具识别性的主体、轮廓、结构、姿态与叙事关系，重新做美术判断并概括成一枚小型 centered vignette / visual island。不要完整复制照片，不要逐物转绘，大胆删除背景与次要信息，只留下最能代表原画面精神、走势和视觉记忆点的少量元素；即使原图构图杂乱、主体过小，也允许重新裁切、组合、缩放和安排位置，使它成为一张独立成立的作品
...
整体呈现极大量白纸、小型视觉岛、鲜活干性手绘色块、粗粝颗粒、不规则边缘与微型手写文字共同构成的高级治愈感。重点不是“把照片画小”，而是把原来的完整世界重新提炼成白纸中央一段最值得留下的记忆。

## 四种输出模式

- `top-bottom`：整张画布只有上下两个全宽区域，现实照片在上、设计在下，严格各占 50%。
- `left-right`：整张画布只有左右两个全高区域，现实照片在左、设计在右，严格各占 50%，不会旋转成上下结构。
- `design-only`：整张画布只呈现 Panel 140 的设计转译，照片只作为不可见参考。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-140`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-140 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-140-prompt.en.md) 与 [中文](references/xxd-panel-140-prompt.zh-CN.md)。
